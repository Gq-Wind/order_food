		...
		<view :style='{"width":"100%","padding":"0","position":"relative","background":"#fff","height":"100%"}'>
			<view class="cu-bar bg-white search" :style='{"width":"100%","padding":"20rpx 24rpx","background":"#fff","display":"flex","height":"auto"}'>
				...
				<view :style='{"margin":"0 12rpx","flex":"1","position":"relative"}' v-if="queryIndex==0" class="search-form round">
					...
					<input :style='{"border":"2rpx solid #A1A1A1","padding":"12rpx 20rpx 12rpx 80rpx","color":"#000","borderRadius":"10rpx","background":"#fff","width":"100%","lineHeight":"56rpx","fontSize":"28rpx","height":"80rpx"}' v-model="searchForm.biaoti" type="text" placeholder="标题" ></input>
				</view>
				<view :style='{"margin":"0 12rpx","flex":"1","position":"relative"}' v-if="queryIndex==1" class="search-form round">
					<text class="icon iconfont icon-fangdajing07" :style='{"color":"#A1A1A1","left":"0px","textAlign":"center","width":"80rpx","fontSize":"40rpx","lineHeight":"80rpx","position":"absolute","right":"0px"}'></text>
					<input :style='{"border":"2rpx solid #A1A1A1","padding":"12rpx 20rpx 12rpx 80rpx","color":"#000","borderRadius":"10rpx","background":"#fff","width":"100%","lineHeight":"56rpx","fontSize":"28rpx","height":"80rpx"}' v-model="searchForm.jianjie" type="text" placeholder="简介" ></input>
				</view>
			...
			<view class="list" :style='{"width":"100%","padding":"24rpx","margin":"0","height":"auto"}'>
				<view @tap="onDetailTap(product)" class="listm flex flex-between" :style='{"boxShadow":"0 2rpx 12rpx rgba(0,0,0,.3)","padding":"20rpx 0 20rpx 20rpx","margin":"0 0 20rpx","overflow":"hidden","borderRadius":"10rpx","flexWrap":"wrap","flexDirection":"column","display":"flex","width":"100%","height":"340rpx"}' v-for="(product,index) in list" :key="index">
					<image :style='{"width":"260rpx","objectFit":"cover","borderRadius":"10rpx","display":"block","height":"300rpx"}' mode="aspectFill" class="listmpic" v-if="preHttp(product.fengmian)" :src="product.fengmian.split(',')[0]"></image>
					<image :style='{"width":"260rpx","objectFit":"cover","borderRadius":"10rpx","display":"block","height":"300rpx"}' mode="aspectFill" class="listmpic" v-else :src="product.fengmian?baseUrl+product.fengmian.split(',')[0]:''"></image>

					<view class="listmr" :style='{"width":"calc(100% - 260rpx)","padding":"0 20rpx","margin":"0","flexWrap":"wrap","display":"flex","height":"auto"}'>
						<view class="col3 f30 elip mb15" :style='{"padding":"0 0 12rpx","margin":"0","whiteSpace":"nowrap","overflow":"hidden","color":"#000","width":"100%","lineHeight":"48rpx","fontSize":"28rpx","textOverflow":"ellipsis","order":"1"}'>{{product.biaoti}}</view>
						<view class="col3 f30 elip mb15" :style='{"padding":"0 0 12rpx","margin":"0","whiteSpace":"nowrap","overflow":"hidden","color":"#000","width":"100%","lineHeight":"48rpx","fontSize":"28rpx","textOverflow":"ellipsis","order":"1"}'>{{product.fabushijian}}</view>
					...
					<view :style='{"width":"calc(100% - 260rpx)","padding":"20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
						<view :style='{"border":"2rpx solid #000","padding":"4rpx 20rpx","borderRadius":"40rpx","display":"flex"}' v-if="(userid && isAuth('gonggaoxinxi','修改')) || (!userid && isAuthFront('gonggaoxinxi','修改'))" @tap.stop.proevent="onUpdate" :data-row="product">
						...
						<view :style='{"border":"2rpx solid #000","padding":"4rpx 20rpx","borderRadius":"40rpx","display":"flex"}' v-if="(userid && isAuth('gonggaoxinxi','删除')) || (!userid && isAuthFront('gonggaoxinxi','删除'))" @tap.stop.proevent="onDelete" :data-row="product">
					...
					<!-- #ifndef MP-WEIXIN -->
					<view :style='{"width":"calc(100% - 260rpx)","padding":"20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
						<view :style='{"border":"2rpx solid #000","padding":"4rpx 20rpx","borderRadius":"40rpx","display":"flex"}' v-if="(userid && isAuth('gonggaoxinxi','修改')) || (!userid && isAuthFront('gonggaoxinxi','修改'))" @tap.stop.proevent="onUpdateTap(product)">
							<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#000","display":"inline-block"}' class="cuIcon-edit"></text>
							<text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#000","display":"inline-block"}'>修改</text>
						</view>
						<view :style='{"border":"2rpx solid #000","padding":"4rpx 20rpx","borderRadius":"40rpx","display":"flex"}' v-if="(userid && isAuth('gonggaoxinxi','删除')) || (!userid && isAuthFront('gonggaoxinxi','删除'))" @tap.stop.proevent="onDeleteTap(product.id)">
							<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#000","display":"inline-block"}' class="cuIcon-delete"></text>
							<text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#000","display":"inline-block"}'>删除</text>
						</view>
					</view>
	...
	export default {
		...
		methods: {
			...
			async search(){
				...
				if(this.searchForm.jianjie){
					searchForm['jianjie'] = '%' + this.searchForm.jianjie + '%'
				}
                let res = {};
                if(this.userid) {
