<template>
	<div class="addEdit-block" :style='{"width":"100%","padding":"0px 30px 30px","fontSize":"14px","color":"#000","background":"none"}'>
		...
		>
			<template >
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="input" v-if="type!='info'"  label="餐品名称" prop="canpinmingcheng">
					<el-input v-model="ruleForm.canpinmingcheng" placeholder="餐品名称" clearable  :readonly="ro.canpinmingcheng"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-else class="input" label="餐品名称" prop="canpinmingcheng">
					<el-input v-model="ruleForm.canpinmingcheng" placeholder="餐品名称" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="select" v-if="type!='info'"  label="餐厅名称" prop="cantingmingcheng">
					<el-select :disabled="ro.cantingmingcheng" v-model="ruleForm.cantingmingcheng" placeholder="请选择餐厅名称" >
						<el-option
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-else class="input" label="餐厅名称" prop="cantingmingcheng">
					<el-input v-model="ruleForm.cantingmingcheng"
						placeholder="餐厅名称" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="select" v-if="type!='info'"  label="餐品分类" prop="canpinfenlei">
					<el-select :disabled="ro.canpinfenlei" v-model="ruleForm.canpinfenlei" placeholder="请选择餐品分类" >
						<el-option
							v-for="(item,index) in canpinfenleiOptions"
							v-bind:key="index"
							:label="item"
							:value="item">
						</el-option>
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-else class="input" label="餐品分类" prop="canpinfenlei">
					<el-input v-model="ruleForm.canpinfenlei"
						placeholder="餐品分类" readonly></el-input>
				</el-form-item>
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="input" v-if="type!='info'"  label="份量" prop="fenliang">
					<el-input v-model="ruleForm.fenliang" placeholder="份量" clearable  :readonly="ro.fenliang"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-else class="input" label="份量" prop="fenliang">
					<el-input v-model="ruleForm.fenliang" placeholder="份量" readonly></el-input>
				</el-form-item>
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-if="type!='info'"  label="餐品介绍" prop="canpinjieshao">
					<editor 
						style="min-width: 200px; max-width: 600px;"
						v-model="ruleForm.canpinjieshao" 
						class="editor" 
						action="file/upload">
					</editor>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-else-if="ruleForm.canpinjieshao" label="餐品介绍" prop="canpinjieshao">
                    <span :style='{"padding":"0 20px","color":"inherit","background":"none","display":"inline-block","fontSize":"14px","lineHeight":"40px","fontWeight":"500"}' v-html="ruleForm.canpinjieshao"></span>
                </el-form-item>
	...
	data() {
		...
		return {
			...
			rules: {
				...
				canpinfenlei: [
					{ required: true, message: '餐品分类不能为空', trigger: 'blur' },
				],
				kouwei: [
				],
				fenliang: [
	...
	methods: {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						if(o=='canpinmingcheng'){
							this.ruleForm.canpinmingcheng = obj[o];
							this.ro.canpinmingcheng = true;
							continue;
						}
						...
						if(o=='canpinfenlei'){
							this.ruleForm.canpinfenlei = obj[o];
							this.ro.canpinfenlei = true;
							continue;
						}
						if(o=='kouwei'){
						...
						if(o=='canpinjieshao'){
							this.ruleForm.canpinjieshao = obj[o];
							this.ro.canpinjieshao = true;
							continue;
						}
			...
			this.$http({
				...
				if (data && data.code === 0) {
					var json = data.data;
					if(((json.cantingmingcheng!=''&&json.cantingmingcheng) || json.cantingmingcheng==0) && this.$storage.get("role")!="管理员"){
            ...
            this.$http({
				url: `option/shangjia/cantingmingcheng`,
				method: "get"
            }).then(({ data }) => {
				if (data && data.code === 0) {
            ...
            this.$http({
				url: `option/canpinfenlei/canpinfenlei`,
				method: "get"
            }).then(({ data }) => {
				if (data && data.code === 0) {
					this.canpinfenleiOptions = data.data;
				} else {
					this.$message.error(data.msg);
				}
            });
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `canpinxinxi/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.canpinxinxiCrossAddOrUpdateFlag = false;
											this.parent.search();
